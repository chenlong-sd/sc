...
- `filters()` 既可以描述筛选 UI，也可以直接在字段上声明查询协议；如果不写，会按列上的 `searchable()` 自动生成默认筛选项
...
- 未显式勾选时，“全部”会清空对应筛选值
- 后端真实字段映射继续通过筛选字段上的 `searchable()/searchField()` 或列上的 `Column::searchable()` 定义，不再塞进 status toggle 自身参数里
...
- `Displays::descriptions()->item(...)` 支持给单个 `el-descriptions-item` 设置属性：第三参可传属性数组或配置回调；也可以先用 `Displays::descriptionItem(...)` 创建 item 后再链式设置 `span()` / `attr()` / `className()`
    ...
    ->addSection(
        Layouts::grid(
            ...
            Displays::descriptions()
                ...
                ->item('状态', '草稿')
                ->item('作者', 'system')
                ->item('备注', '这项占两列', ['span' => 2])
                ->item(
                    Displays::descriptionItem('说明', '可单独设置属性')
                        ->span(2)
            ...
            ->filters(
                Forms::make('product-filters')->inline()->addFields(
                    Fields::text('keyword', '关键词')
                        ->placeholder('商品名 / 编码')
                        ->searchable('LIKE', 'name&code'),
                    Fields::select('status', '状态')->options([
                        '' => '全部',
                        1 => '上架',
                        0 => '下架',
                    ]),
                    Fields::datetime('created_at', '创建时间')->pickerType('daterange')
                )
            )
            ->table(
        ...
        Lists::make('user-list')
            ->filters(
                Forms::make('user-filters')->inline()->addFields(
                    Fields::text('keyword', '关键词')
                        ->placeholder('用户名 / 手机号')
                        ->searchable('LIKE', 'username&mobile')
                )
            )
            ->table(
                Tables::make('user-table')
            ...
            ->filters(
                Forms::make('order-filters')->inline()->addFields(
                    Fields::text('keyword', '订单关键词')
                        ->searchable('LIKE', 'order_no&buyer_name')
                )
            )
            ->table(
...
- 默认支持下载 Excel 模板，表头上方自动附带字段说明行（可选值、必填等）
- 默认支持 JSON 导入
- 默认支持复制 AI 测试数据提示词
- 面板内自带导入预览和结果展示
...
Fields::select('status', '状态')
    ->linkageUpdate('status_text', '@label')
    ->on('change', '(value) => console.log(value, model.remark, form.id)');
```
...
- 省略 `filters()` 时，`List` 会尝试按列上的 `searchable()` 自动生成默认筛选项；这些自动项默认隐藏 label，只保留 placeholder；已显式声明的字段会保留，其余缺失项继续自动补齐
...
- `Fields::date()`、`Fields::datetime()`、`Fields::daterange()` 才暴露 `format()`、`valueFormat()`、`pickerType()` / `setTimeType()`
- `Fields::datetime()` 现在是统一时间选择入口，可通过 `pickerType('date' | 'datetime' | 'daterange' | 'datetimerange' | 'time')` 切换模式
...
- 筛选协议优先归 `filters()` 字段定义，列上的 `searchable()` 作为自动推导和补齐入口；不写 `filters()` 时会按 `searchable()` 自动生成默认筛选表单，写了时也会自动补齐缺失筛选项
